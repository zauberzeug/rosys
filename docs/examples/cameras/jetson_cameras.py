...
Run on the Jetson with the project's Python (not `uv run`, which re-executes the script):
``./.venv/bin/python docs/examples/cameras/jetson_cameras.py``
"""
...
def main() -> None:
    ...
    ui.timer(0.1, update)
    ui.run(title='RoSys GMSL cameras', reload=False)


if __name__ == '__main__':
    multiprocessing.set_start_method('spawn', force=True)  # required by rosys; normally provided by reload mode
    main()
