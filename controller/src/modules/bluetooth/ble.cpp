...
static std::string DEVICE_NAME = "ZCam";
static void (*HANDLE_COMMAND)(std::string);

#define MAX_CHAR_VALUE_LENGTH 0x40
...
static esp_attr_value_t gatts_demo_char1_val =
    {
        .attr_max_len = MAX_CHAR_VALUE_LENGTH,
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_REG_EVT:
    {
        ...
        adv_config_done |= scan_rsp_config_flag;
        esp_ble_gatts_create_service(gatts_if, &gl_profile.service_id, NUM_HANDLE);
        break;
    }
    ...
    case ESP_GATTS_WRITE_EVT:
    {
        ...
        if (!param->write.is_prep)
        {
            std::string msg = (char *)param->write.value;
            ESP_LOGI(TAG, "GATT_WRITE_EVT, len %d, value: %s", param->write.len, msg.c_str());
            HANDLE_COMMAND(msg);
...
void Ble::init(std::string device_name, void (*handleCommand)(std::string))
{
