...
from ..hardware import AppButton, AppControls
    ...
    def __init__(self,
        ...
        async def start():
            if inspect.iscoroutinefunction(can_start):
                ...
                return
            automator.start(default_automation())

        def pause():
            automator.pause(because='pause button was pressed')
        ...
        play_button = ui.button(on_click=start).props('icon=play_arrow unelevated').tooltip('start automation')
        pause_button = ui.button(on_click=pause).props('icon=pause outline').tooltip('stop automation')
        resume_button = ui.button(on_click=resume).props('icon=play_arrow outline').tooltip('resume automation')
        stop_button = ui.button(on_click=stop).props('icon=stop outline').tooltip('stop automation')
        ...
        async def refresh():
            ...
            play_button.view.disable = default_automation is None or not automator.enabled
        ...
        ui.timer(settings.update_interval, refresh)
