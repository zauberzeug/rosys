...
class RobotObject(Object3D):
    ...
    def __init__(self, robot: Robot, *, show_hook: bool = False):
        super().__init__('group')
        self.robot = robot
        with self:
            outline = list(map(list, robot.shape.outline))
            Extrusion(outline, robot.shape.height).material('#4488ff', 0.5)
            if show_hook:
                Sphere(0.05).move(robot.parameters.hook_offset).material('#4488ff', 0.5)
        self.update()
