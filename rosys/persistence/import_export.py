...
def export_all(path: Path | None = None, indent: int = 4) -> dict[str, Any]:
...
def import_all(source: Path | dict[str, Any]) -> None:
...
def export_button(title: str = 'Export', indent: int = 4) -> ui.button:
    return ui.button(title, on_click=lambda: ui.download.content(json.dumps(export_all(), indent=indent),
