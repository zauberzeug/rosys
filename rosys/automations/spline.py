...
async def drive_spline(spline: Spline, world: World, esp: Esp, *, backward: bool = False, throttle_at_end: bool = True):
    ...
    while True:
        ...
        if is_offset:
            ...
            if backward:
                raise NotImplementedError('Driving backwards is only supported for centered points-of-interest.')
            ...
            try:
                ...
                break
            curvature = local_spline.max_curvature(0.0, 0.25) * (-1 if backward else 1)
            backward = backward
