...
async def drive_path(world: World, wheels: Wheels, path: list[PathSegment]):
    for segment in path:
        await drive_spline(segment.spline, world, wheels, throttle_at_end=segment == path[-1], flip_hook=segment.backward)


async def drive_to(world: World, wheels: Wheels, target: Point):
    if world.robot.parameters.minimum_turning_radius:
        await drive_circle(world, wheels, target)
    ...
    await drive_spline(approach_spline, world, wheels)


async def drive_circle(world: World, wheels: Wheels, target: Point):
    while True:
        ...
        await wheels.drive(*throttle(world, linear, angular))
        await sleep(0.1)
