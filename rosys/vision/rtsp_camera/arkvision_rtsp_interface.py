...
class ArkVisionRtspInterface:
    """Reads and writes ArkCam Basic+ H.264 stream settings through its REST API (``http://<ip>/rest/...``).

    The ArkCam exposes a single H.264 stream, so the ``stream_id`` argument (kept for interface
    compatibility with the other RTSP settings interfaces) is ignored.  Getters return ``None`` on a
    REST error so the caller can keep the previously cached value.
    """

    def __init__(self, ip: str) -> None:
        self.ip = ip
        self.log = logging.getLogger('rosys.vision.rtsp_camera.arkvision_rtsp_interface')
    ...
    async def set_fps(self, stream_id: int, fps: int) -> None:  # pylint: disable=unused-argument
        ...
        await self._post('streamH264', {'framerate': index})
