...
class Schedule(persistence.PersistentModule):
    ...
    def update_sun_limits(self) -> None:
        if self.location:
            ...
            start_time = sun.get_sunrise_time(at_date=datetime.now()) + timedelta(minutes=self.sunrise_offset)
            stop_time = sun.get_sunset_time(at_date=datetime.now()) + timedelta(minutes=self.sunset_offset)
