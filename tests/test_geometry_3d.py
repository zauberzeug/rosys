...
def test_resolve_frames():
    ...
    A = Pose3d(translation=Point3d(x=3, y=0, z=0), rotation=turn_left).as_frame('A')
    B = Pose3d(translation=Point3d(x=2, y=0, z=0), rotation=turn_right).as_frame('B').in_frame(A)

    P = Pose3d(translation=Point3d(x=1, y=1, z=0), rotation=turn_right).in_frame(B)
